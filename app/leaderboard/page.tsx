      ...
      <div className="space-y-2">
        {isLoading ? <p className="text-[13px] text-[#6b5346]">טוען נתונים...</p> : rows.length === 0 ? (
          ...
          return (
            <div
              key={row.id}
              className={`flex items-center gap-3 rounded-[12px] p-3 ${isTopThree ? "border border-[#d98a6b] bg-[#F5C4B3]" : "bg-[#fff7f2]"}`}
            >
              <span className="w-[20px] text-[14px] font-medium text-[#2d120b]">{index + 1}</span>
              <div className={`flex h-[32px] w-[32px] items-center justify-center rounded-full text-[12px] font-medium ${isTopThree ? "bg-[#D85A30] text-[#fff]" : "bg-[#f0dfd5] text-[#2d120b]"}`}>
